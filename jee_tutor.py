...
if st.session_state.logged_in:
    ...
    db_execute(
        "UPDATE users SET referral_code = ? WHERE username = ?", (code, uname))
...
# --- 2. UI & CSS ---
st.set_page_config(page_title="Arjun | JEE Mentor",
...
if 'user_data' not in st.session_state:
    data = db_execute("SELECT tos_agreed, pro_expiry, user_summary, referral_code FROM users WHERE username = ?",
                      (st.session_state.username,), fetchone=True)
    if data:
        # If the referral code is missing in DB, generate and save it
        if not data[3]:
            ref_code = hashlib.sha256(
                st.session_state.username.encode()).hexdigest()[:6].upper()
            db_execute("UPDATE users SET referral_code = ? WHERE username = ?",
                       (ref_code, st.session_state.username))
            data = (data[0], data[1], data[2], ref_code)
    else:
        # New user entry
        ref_code = hashlib.sha256(
            st.session_state.username.encode()).hexdigest()[:6].upper()
        db_execute("INSERT INTO users (username, referral_code) VALUES (?, ?)",
                   (st.session_state.username, ref_code))
        data = (0, None, "New student. Let's build your AIR profile.", ref_code)
    st.session_state.user_data = data

if 'feedback_submitted' not in st.session_state:
    st.session_state.feedback_submitted = bool(db_execute(
        "SELECT 1 FROM feedback WHERE username = ?", (st.session_state.username,), fetchone=True))
...
def process_referral(current_user, entered_code):
    ...
    if entered_code == "FOUNDER_BETA_2026":
        db_execute(
            "UPDATE users SET pro_expiry = '2027-01-01' WHERE username = ?", (current_user,))
    ...
    referrer = db_execute(
        "SELECT username FROM users WHERE referral_code = ?", (entered_code,), fetchone=True)
    ...
    db_execute("INSERT OR IGNORE INTO users (username) VALUES (?)",
               (referrer_username,))
    ...
    db_execute("INSERT INTO referrals (referrer_id, referee_id, timestamp) VALUES (?, ?, ?)",
               (referrer_username, current_user, datetime.now()))
...
if not st.session_state.tos_confirmed:
    ...
    def tos_dialog():
        ...
        if st.button("I Agree & Start Learning", type="primary", use_container_width=True):
            db_execute("UPDATE users SET tos_agreed = 1 WHERE username = ?",
...
# --- 5. DASHBOARD DATA ---
is_pro = bool(user_info and user_info[1] and datetime.strptime(
    user_info[1], '%Y-%m-%d').date() >= date.today())
...
def show_pricing():
    ...
    with c1:
        if st.button("Monthly - ₹499"):
            db_execute("UPDATE users SET payment_intent = 'Monthly' WHERE username = ?",
                       (st.session_state.username,))
            st.toast("Team notified!")
    with c2:
        if st.button("Yearly - ₹3,999", type="primary"):
            db_execute("UPDATE users SET payment_intent = 'Yearly' WHERE username = ?",
                       (st.session_state.username,))
            st.toast("Team notified!")
...
with col_dash:
    ...
    with st.container(border=True):
        ...
        if not st.session_state.feedback_submitted:
            f_text = st.text_area("How is Arjun helping you today?",
                                  placeholder="My first impression is...", key="f_input")
    ...
    with st.container(border=True):
        if not st.session_state.feedback_submitted:
            ...
            st.info(
                "Unlock your Referral Code by sharing your first impression of Arjun.")
        else:
            st.write("🎁 **Referral Code**")
            ...
            ref_input = st.text_input(
                "Friend's Code", placeholder="Enter code...", key="ref_input")
    ...
    with st.expander("⚙️ Admin Console"):
        if st.text_input("Passcode", type="password") == "FOUNDER_BETA_2026":
            ...
            st.dataframe(pd.read_sql_query(
                "SELECT username, payment_intent FROM users WHERE payment_intent IS NOT NULL", conn))
            st.dataframe(pd.read_sql_query(
                "SELECT username, referral_code, pro_expiry, total_referrals FROM users", conn))
...
if prompt := st.chat_input("Ask about Physics, Chemistry, or Math..."):
    ...
    with col_chat:
        ...
        with st.chat_message("assistant"):
            ...
            api_messages = [{"role": "system", "content": sys}] + [{"role": m["role"],
                                                                    "content": m["content"]} for m in st.session_state.messages if m["content"].strip()]
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    ...
                    if "429" in str(e) and attempt < max_retries - 1:
                        ...
                        st.error(
                            "Arjun is thinking deeply (Rate Limit). Try again in 10 seconds.")
                        break
    st.rerun()
